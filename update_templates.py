...
REPLACEMENTS = [
    ...
    (
        r'<link href="https://cdn\.bootcdn\.net/ajax/libs/bootstrap/5\.3\.0/css/bootstrap\.min\.css" rel="stylesheet">',
        '<link href="{{ url_for(\'static\', filename=\'libs/bootstrap/css/bootstrap.min.css\') }}" rel="stylesheet">'
    ),
    # Bootcdn Bootstrap JS
    (
        r'<script src="https://cdn\.bootcdn\.net/ajax/libs/bootstrap/5\.3\.0/js/bootstrap\.bundle\.min\.js"></script>',
        '<script src="{{ url_for(\'static\', filename=\'libs/bootstrap/js/bootstrap.bundle.min.js\') }}"></script>'
    ),
    # Bootcdn Font Awesome
    (
        r'<link href="https://cdn\.bootcdn\.net/ajax/libs/font-awesome/6\.4\.0/css/all\.min\.css" rel="stylesheet">',
        '<link rel="stylesheet" href="{{ url_for(\'static\', filename=\'libs/fontawesome/css/all.min.css\') }}">'
    ),
]
...
def main():
    ...
    print()
    print("下一步:")
    print("1. 运行 python download_libs.py 下载依赖库")
    print("2. 重启 Flask 应用")
    print("3. 刷新浏览器页面")
